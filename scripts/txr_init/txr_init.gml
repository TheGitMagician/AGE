function txr_init() {
	...
	enum txr_action {
		...
		ident = 2, // (name): push(self[name]) - currently this doesn't use self but a central object that manages all variables (set via txr_thread.scope)
		...
		set_ident = 11, // (name:string): self[name] = pop() - currently this doesn't use self but a central object that manages all variables (set via txr_thread.scope)
	...
	//store the current context from which a field has been gotten (via txr_thread_resume() -> txr_action.get_field)
	//necessary so that TXR can not only access variables/functions of the local instance but also of structs that are stored in the current instance
	global.txr_exec_context = undefined; 
}
// GMEdit hints:
